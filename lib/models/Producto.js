...
module.exports = class Producto extends Schwifty.Model {
    ...
    static buscarProducto(idEmpresa, impuestos, busqueda) {

        const terminoBusqueda = `%${busqueda}%`;

        return this
            .query()
            .eagerAlgorithm(Producto.JoinEagerAlgorithm)
            .eager('[tipo_producto.impuestos, unidad_medida]')
            .where('id_empresa', idEmpresa)
            .andWhere((builder) => {

                return builder.whereIn('tipo_producto:impuestos.id_impuesto', impuestos);
            })
            .andWhere((builder) => {
