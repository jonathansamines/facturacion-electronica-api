...
module.exports = class TipoDocumento extends Schwifty.Model {
    ...
    static async obtenerIdImpuestosPorId(idTipoDocumento) {

        const tipoDocumento = await this.query()
            .eagerAlgorithm(TipoDocumento.JoinEagerAlgorithm)
            .eager('impuestos(idImpuesto)', {
                idImpuesto: (builder) => builder.select('id_impuesto')
            })
            .findById(idTipoDocumento);

        return tipoDocumento.impuestos.map((impuesto) => impuesto.id_impuesto);
    }
